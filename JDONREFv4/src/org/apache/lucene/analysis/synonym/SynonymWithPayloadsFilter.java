...
/**
 * Synonym extension for payloads 
 * 
 * @author moquetju
 */
public final class SynonymWithPayloadsFilter extends TokenFilter {
  ...
  private static class PendingOutputs {
    ...
    public void add(char[] output, int offset, int len, int endOffset, int posLength, byte[] payload) {
   ...
   */
  public SynonymWithPayloadsFilter(TokenStream input, SynonymMap synonyms, boolean ignoreCase) {
    super(input);
  ...
  */
  ...
  private void parse() throws IOException {
      ...
      if (curNextRead == nextWrite) {
        ...
        if (finished) {
          ...
          if (input.incrementToken()) {
            ...
            input.payload = payload!=null?payload.bytes:null;
        ...
        payload = futureInputs[curNextRead].payload==null?null:new BytesRef(futureInputs[curNextRead].payload);
    ...
    if (matchOutput != null) {
      ...
      addOutput(matchOutput, matchInputLength, matchEndOffset,payload!=null?payload.bytes:null);
  ...
  private void addOutput(BytesRef bytes, int matchInputLength, int matchEndOffset, byte[] payload) {
    ...
    for(int outputIDX=0;outputIDX<count;outputIDX++) {
      ...
      for(int chIDX=lastStart;chIDX<=chEnd;chIDX++) {
        if (chIDX == chEnd || scratchChars.chars[chIDX] == SynonymMap.WORD_SEPARATOR) {
          ...
          futureOutputs[outputUpto].add(scratchChars.chars, lastStart, outputLen, endOffset, posLen, payload);
    ...
    while(true) {
      ...
      while (inputSkipCount != 0) {
        ...
        if (!input.consumed && (input.keepOrig || !input.matched)) {
          ...
          // position
          input.reset();
          final byte[] payload = outputs.getPayload();
