 ...
 * @author Julien
 */
public class IntegerTypeAsPayloadFilterTests extends QueryTests
{
    public IntegerTypeAsPayloadFilterTests()
    {
        settingsFileName = "./test/resources/index/IntegerTypeAsPayloadFilter-settings.json";
        INDEX_NAME = "test";
        DOC_TYPE_NAME = "test";
    }
    ...
    {
        String[] mappings = new String[]{"integertypeaspayloadfilter"};
    ...
    void index() throws IOException, InterruptedException, ExecutionException
    {
        ...
        publicIndex(brb,"integertypeaspayloadfilter","1",XContentFactory.jsonBuilder().startObject()
                .field("fullName","AA|1 BB|1 CC|2 DD|3")
                .endObject());
        publicIndex(brb,"integertypeaspayloadfilter","2",XContentFactory.jsonBuilder().startObject()
    ...
    public void testSearch() throws IOException, InterruptedException, ExecutionException
    {
        ...
        TermVectorResponse response1 = client().prepareTermVector(INDEX_NAME, "integertypeaspayloadfilter", "1").get();
        ...
        while((terms1.next())!=null)
        {
            ...
            while(docsEnum.nextDoc()!=DocsAndPositionsEnum.NO_MORE_DOCS) // no need for nextPosition for these samples
            {
                ...
                while(count++<freq)
                {
                    ...
                    assert(payloadValue==1002001 || payloadValue==1001002 || payloadValue==1001003
                        || payloadValue==2002001 || payloadValue==2001002 || payloadValue==2001003);
                }
            }
        }
        
        TermVectorResponse response2 = client().prepareTermVector(INDEX_NAME, "integertypeaspayloadfilter", "2").get();
        ...
        while((terms2.next())!=null)
        {
            ...
            while(docsEnum.nextDoc()!=DocsAndPositionsEnum.NO_MORE_DOCS) // no need for nextPosition for these samples
            {
                ...
                while(count++<freq)
                {
                    ...
                    BytesRef payload = docsEnum.getPayload();
                    int payloadValue = PayloadHelper.decodeInt(payload.bytes,payload.offset);
                    assert(payloadValue==1001001 || payloadValue==1001002 || payloadValue==1001003
                        || payloadValue==2001001 || payloadValue==2001002 || payloadValue==2001003);
                }
            }
        }
    }
