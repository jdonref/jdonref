...
import ppol.jdonref.json.client.ResultatGeocodageInverse;
...
public class Reverse extends HttpServlet {
     ...
     * Le service web spécifié dans le paramètre JDONREFv2Inversewsdl est utilisé.<br>
     ...
     *
     * {"etat":etat,"codeerreur":code,"message":"message"} en cas d'erreur
     * ou {"etat":1,"compte":compte,"propositions":[ {"precision":precision,"ligne1":"ligne1",...,"ligne6":"ligne6",
     * "x":"x","y":"y","t0":"t0","t1":"t1","distance":"distance"}, ... ]}
     *
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultatGeocodageInverse resultat = new ResultatGeocodageInverse();
...
//        }
        ...
        final String distanceParam = (request.getParameter("distance") != null) ? URLDecoder.decode(request.getParameter("distance").trim(), "UTF-8") : "0.5";
        ...
        for (String serviceStr : servicesParam) {
            try {
                services.add(Integer.parseInt(new String(serviceStr.getBytes("UTF-8"), "UTF-8").trim()));
        ...
        try {
            ...
            out.print("{\"etat\":0,\"codeerreur\":5,\"message\":\"La distance est invalide.\"}");
            out.close();
            return;
        }
        ...
        if (config != null && config.getServletContext().getInitParameter("debug") != null) {
            ...
            Logger.getLogger("Inverse").log(Level.INFO, sb.toString());
        }

        try {
             // INVOCATION DU SERVICE
            final JDONREF port = service.getJDONREFPort();
            resultat = port.reverse(application, services, donnees, distance, options);
            
            // TRAITEMENT DE LA REPONSE
            if (resultat == null) {
                Logger.getLogger("Inverse").log(Level.SEVERE, "Le service web ne répond pas.");
                resultat = new ResultatGeocodageInverse();
                resultat.setCodeRetour(0);
                resultat.getErreurs().add(Commons.getErreurServiceSansReponse());
            }
        } catch (Exception ex) {
            Commons.log("Inverse", Level.SEVERE, ex);
            resultat.getErreurs().add(Commons.getResultatErreur(7, "Erreur durant le géocodage inverse."));
        } finally {
            Commons.printJSONResponse(out, resultat);
        }
    }
