...
import ppol.jdonref.json.client.ResultatContacte;
...
public class Contacte extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultatContacte resultat = new ResultatContacte();
        ...
        if (config != null && config.getServletContext().getInitParameter("debug") != null) {
            ...
            Logger.getLogger("Contacte").log(Level.INFO, sb.toString());
        }

        // EXECUTION DU SERVICE
        try {
            final JDONREF port = service.getJDONREFPort();
            resultat = port.contacte(application, services, operation, donnees, options);

            // TRAITEMENT DE LA REPONSE
            if (resultat == null) {
                Logger.getLogger("Contacte").log(Level.SEVERE, "Le service web ne répond pas.");
                resultat = new ResultatContacte();
                resultat.setCodeRetour(0);
                resultat.getErreurs().add(Commons.getErreurServiceSansReponse());
            }
        } catch (Exception ex) {
            Commons.log("Contacte", Level.SEVERE, ex);
            resultat.getErreurs().add(Commons.getResultatErreur(7, "Erreur durant le contacte."));
        } finally {
            Commons.printJSONResponse(out, resultat);
        }
    }
