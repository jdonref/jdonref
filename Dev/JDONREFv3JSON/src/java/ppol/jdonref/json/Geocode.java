...
import ppol.jdonref.json.client.ResultatGeocodage;
...
public class Geocode extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultatGeocodage resultat = new ResultatGeocodage();
...
//        }
        ...
        if (config != null && config.getServletContext().getInitParameter("debug") != null) {
            ...
            Logger.getLogger("Geocode").log(Level.INFO, sb.toString());
        }
        
        // EXECUTION DU SERVICE
        try {
            final JDONREF port = service.getJDONREFPort();
            resultat = port.geocode(application, services, donnees, ids, options);

            // TRAITEMENT DE LA REPONSE// TRAITEMENT DE LA REPONSE
            if (resultat == null) {
                Logger.getLogger("Geocode").log(Level.SEVERE, "Le service web ne répond pas.");
                resultat = new ResultatGeocodage();
                resultat.setCodeRetour(0);
                resultat.getErreurs().add(Commons.getErreurServiceSansReponse());
            }
        } catch (Exception ex) {
            Commons.log("Geocode", Level.SEVERE, ex);
            resultat.getErreurs().add(Commons.getResultatErreur(7, "Erreur durant le géocodage."));
        } finally {
            Commons.printJSONResponse(out, resultat);
        }
    }
