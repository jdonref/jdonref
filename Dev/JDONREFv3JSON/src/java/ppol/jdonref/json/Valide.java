...
import ppol.jdonref.json.client.ResultatValidation;
...
//        }
        ...
        final String operationParam = (request.getParameter("operation") != null) ? URLDecoder.decode(request.getParameter("operation").trim(), "UTF-8") : "0";
        ...
        if (config != null && config.getServletContext().getInitParameter("debug") != null) {
            ...
            Logger.getLogger("Valide").log(Level.INFO, sb.toString());
        }

        // EXECUTION DU SERVICE
        try {
            final JDONREF port = service.getJDONREFPort();
            resultat = port.valide(application, services, operation, donnees, ids, options);

            // TRAITEMENT DE LA REPONSE
            if (resultat == null) {
                Logger.getLogger("Valide").log(Level.SEVERE, "Le service web ne répond pas.");
                resultat = new ResultatValidation();
                resultat.setCodeRetour(0);
                resultat.getErreurs().add(Commons.getErreurServiceSansReponse());
            }
        } catch (Exception ex) {
            Commons.log("Valide", Level.SEVERE, ex);
            resultat.getErreurs().add(Commons.getResultatErreur(7, "Erreur durant la validation."));
        } finally {
            Commons.printJSONResponse(out, resultat);
        }
    }
