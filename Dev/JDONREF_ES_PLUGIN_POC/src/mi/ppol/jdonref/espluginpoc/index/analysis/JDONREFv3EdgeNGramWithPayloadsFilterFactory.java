package mi.ppol.jdonref.espluginpoc.index.analysis;
 ...
 * @author Julien
 */
public class JDONREFv3EdgeNGramWithPayloadsFilterFactory extends EdgeNGramTokenFilterFactory
{
  protected boolean withPayloads = false;
  protected int nonPrivateMinGramSize;
  protected int nonPrivateMaxGramSize;
  protected JDONREFv3EdgeNGramWithPayloadsFilter.Side nonPrivateSide;
  protected org.elasticsearch.Version nonPrivateEsVersion;
  
  /** Creates a new JDONREFv3EdgeNGramWithPayloadsFilterFactory */
  @Inject
  public JDONREFv3EdgeNGramWithPayloadsFilterFactory(Index index, @IndexSettings Settings indexSettings, @Assisted String name, @Assisted Settings settings)
  {
    ...
    nonPrivateSide = JDONREFv3EdgeNGramWithPayloadsFilter.Side.getSide(settings.get("side", Lucene43EdgeNGramTokenizer.DEFAULT_SIDE.getLabel()));
  ...
  public TokenStream create(TokenStream tokenStream) {
      ...
      if (version.onOrAfter(Version.LUCENE_43) && nonPrivateEsVersion.onOrAfter(org.elasticsearch.Version.V_0_90_2))
      {
            ...
            if (nonPrivateSide == JDONREFv3EdgeNGramWithPayloadsFilter.Side.BACK) {
                result = new ReverseStringFilter(version, result);
            }
            result = new JDONREFv3EdgeNGramWithPayloadsFilter(version, result, nonPrivateMinGramSize, nonPrivateMaxGramSize, withPayloads);
            if (nonPrivateSide == JDONREFv3EdgeNGramWithPayloadsFilter.Side.BACK) {
                result = new ReverseStringFilter(version, result);
            }
            return result;
      }
      return new JDONREFv3EdgeNGramWithPayloadsFilter(version, tokenStream, nonPrivateSide, nonPrivateMinGramSize, nonPrivateMaxGramSize, withPayloads);
  }
}
