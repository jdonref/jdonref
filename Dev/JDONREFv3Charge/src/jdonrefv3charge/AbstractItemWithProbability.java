 ...
 * @author arochewi
 */
public abstract class AbstractItemWithProbability
{
    protected int probabilite;

    /**
     * Permet de lire dans un element DOM XML un item.
     * @param e : org.jdom.Element
     */
    protected void load(Element e) throws Exception
    {
        Element e_nom = e.getChild(getItemXmlDesignation());
        Element e_probabilite = e.getChild("probabilite");

        if((e_nom == null) || (e_probabilite == null))
                throw(new Exception("La structure XML du "+ getItemName() +" est incorrecte"));

        setItemName(e_nom.getValue());
        try
        {
    ...
    {
        return "[" + specificToString() + " probabilite : " + probabilite + "]";
    }
